...
public class BuildCompletionNotifyingBuildActionRunner implements BuildActionRunner {
    ...
    public Result run(final BuildAction action, final BuildController buildController) {
        GradleEnterprisePluginEndOfBuildNotifier endOfBuildNotifier = buildController.getGradle().getServices().get(GradleEnterprisePluginEndOfBuildNotifier.class);
        Result result;
        try {
        ...
        endOfBuildNotifier.buildComplete(result.getBuildFailure());
        return result;
    }
}
