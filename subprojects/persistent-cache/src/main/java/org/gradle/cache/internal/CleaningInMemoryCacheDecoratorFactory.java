...
public class CleaningInMemoryCacheDecoratorFactory extends DefaultInMemoryCacheDecoratorFactory {
    private final List<WeakReference<ClearableInMemoryCache>> inMemoryCaches = Lists.newArrayList();
    ...
    public void clearCaches(Predicate<ClearableInMemoryCache> predicate) {
        for (Iterator<WeakReference<ClearableInMemoryCache>> iterator = inMemoryCaches.iterator(); iterator.hasNext();) {
            WeakReference<ClearableInMemoryCache> ref = iterator.next();
            ClearableInMemoryCache cache = ref.get();
            if (cache == null) {
                iterator.remove();
    ...
    protected <K, V> MultiProcessSafeAsyncPersistentIndexedCache<K, V> applyInMemoryCaching(String cacheId, MultiProcessSafeAsyncPersistentIndexedCache<K, V> backingCache, int maxEntriesToKeepInMemory, boolean cacheInMemoryForShortLivedProcesses) {
        ...
        if (delegate instanceof ClearableInMemoryCache) {
            ClearableInMemoryCache cimc = (ClearableInMemoryCache) delegate;
