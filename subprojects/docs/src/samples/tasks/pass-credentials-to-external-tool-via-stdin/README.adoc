...
This sample shows how credentials can be passed to an external tool that only accepts them via standard input.
...
Gradle build file registers two tasks - one performs the login step and the other one depends on the login having succeeded:
====
include::sample[dir="groovy",files="build.gradle[]"]
include::sample[dir="kotlin",files="build.gradle.kts[]"]
====

The tasks are using link:{userManualPath}/build_environment.html#sec:project_properties[Gradle properties] to capture the required credentials.
...
This allows the sensitive data to be kept outside of the project itself. `gradle.properties` can reside in the user's `~/.gradle` directory and environment variables can be set on CI systems.

The values are also not shown in the build output this way.

For more information about using Gradle properties, see link:{userManualPath}/build_environment.html#sec:gradle_configuration_properties[Gradle Properties in the user manual].
