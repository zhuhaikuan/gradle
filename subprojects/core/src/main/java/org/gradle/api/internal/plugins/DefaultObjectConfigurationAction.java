    ...
    private HttpRedirectVerifier createHttpRedirectVerifier(URI scriptUri) {
        return HttpRedirectVerifierFactory.create(
            ...
            () -> DeprecationLogger
                ...
                .withAdvice("Use '%s' instead or try 'apply from: resources.text.fromInsecureUri(\"%s\")' to silence the warning.", GUtil.toSecureUrl(scriptUri), scriptUri)
                .withContext("The provided URI '%s' uses an insecure protocol (HTTP).", scriptUri)
            ...
            redirect -> DeprecationLogger
                ...
                .withContext("'%s' redirects to insecure '%s'.", scriptUri, redirect)
                .willBeRemovedInGradle7()
                .withDslReference(TextResourceFactory.class, "fromInsecureUri(java.lang.Object)")
                .nagUser()
        );
